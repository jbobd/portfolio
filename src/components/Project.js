...
const Project = (props) => {
  ...
  const listIterate = () => {
    ...
    return (
      <div>
        <h1 className="project__info-title project__skills-head">
          Skills Utilized
        </h1>
        <div className="skills-columns">
          {skills.map((skillType) => {
            ...
            return (
              <ul
                key={`project__skills-${skillType}`}
                className="project__skills-list"
              >
                <li className="project__skills-subhead">
                  <b>{skillType}</b>
                </li>
                {skillList.map((skill) => (
                  <li
                    key={`project__skils-${skill}`}
      ...
      <div className="project__pagination">
        {pages.map((page) => {
          if (page === "info") {
            return (
              <div
                key={`project__pagination-${page}`}
                onClick={handlePage}
                className={`project__pagination-item project__pagination-${page} activePage`}
              />
            );
          }
          return (
            <div
              key={`project__pagination-${page}`}
              onClick={handlePage}
              className={`project__pagination-item project__pagination-${page}`}
            />
          );
        })}
      </div>
  ...
  const renderInfo = (page) => {
    const info = (
      <div className="project__info">
        <h1 className="project__info-title project__info-titleSummary">
          Summary
        </h1>
